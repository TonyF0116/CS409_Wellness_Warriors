...
class DataStore {
  ...
  async getHabit(userId, habitId) {
    await this.ensureReady();
    return (
      this.state.habits.find(
        (habit) => habit.userId === userId && habit.id === habitId
      ) || null
    );
  }
  ...
  async deleteHabit(userId, habitId) {
    await this.ensureReady();
    const habitIndex = this.state.habits.findIndex(
      (habit) => habit.userId === userId && habit.id === habitId
    );
    if (habitIndex === -1) {
      return false;
    ...
    this.state.completions = this.state.completions.filter(
      (completion) =>
        completion.userId !== userId || completion.habitId !== habitId
    );
    await this.persist();
    return true;
  }
  ...
  async addCompletion(userId, habitId, date) {
    await this.ensureReady();
    const existing = this.state.completions.find(
      (c) => c.userId === userId && c.habitId === habitId && c.date === date
    );
    if (existing) {
      return existing;
    }
  ...
  async removeCompletion(userId, habitId, completionId) {
    await this.ensureReady();
    const index = this.state.completions.findIndex(
      (c) =>
